...
for (let i=0; i<starWarsCharacters.length; i++) {
  const pg = starWarsCharacters[i];
  characters.push(pg.name);
}
...
for (let i = 0; i < starWarsCharacters.length; i++) {
  const pgsw = starWarsCharacters[i];

  switch (pgsw.eye_color) {
    case "blue":
      eyeColor.blue.push(pgsw);
      break;
    case "yellow":
      eyeColor.yellow.push(pgsw);
      break;
    case "brown":
      eyeColor.brown.push(pgsw);
      break;
    case "red":
      eyeColor.red.push(pgsw);
      break;
    case "blue-gray":
      eyeColor["blue-gray"].push(pgsw);
      break;
    default:
...
while (i < starWarsCharacters.length) {
  ...
  crewMass += parsenIt(currentCharacter.mass);
  i++;
}
...
switch(true) {
  ...
  break
  case crewMass > 900 && crewass < 1000: console. log('Critical Load: Over 900')
  break
  case crewMass > 1000:
...
for (let i = 0; i < starWarsCharacters.length; i++) {
  const bot = starWarsCharacters[i];
